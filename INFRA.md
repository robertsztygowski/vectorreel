...
| `scripts/produce-collection.sh <slug>` | **The only command that spends inference money on purpose.** Reads a licence-verified `corpus.json`, produces its `full`-tier sources through the real pipeline (`PipelineModel__Mode=live`) from local, and writes a cost-reconciliation report next to the corpus. **Resumable** — a source whose `output.md` is already in `outputs-eu` is skipped, never re-paid for. **Calibrate first:** `--calibrate 2` produces two, then stops, so the measured rate can be reconciled against METRICS.md **N4c** and the batch extrapolated before the rest is committed. Runs blanket **low media resolution** (METRICS.md **N4b**) and carries the rule-9 guards on every Stage B call; a session breaching the per-session ceiling aborts the batch rather than paying the METRICS.md **N4d** over-segmentation rate across the whole corpus. `--dry-run` proves the wiring with zero spend. Deployed services are untouched and stay `PipelineModel__Mode=fake`. |
...
| **Verified** | `/health` 200; `POST /api/v1/events` 202 with Postgres persistence (schema auto-created); **live 2026-07-20: 360 MiB signed PUT straight to GCS (200), fake-mode job done in ~40 s, `output.md` served, `uploads/{uploadId}` object erased after Stage D** |
